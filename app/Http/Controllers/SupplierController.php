...
class SupplierController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $total_receivings = Receiving::where('supplier_id', $id)->count();
        $total_dues = Receiving::where('supplier_id', $id)->sum('dues');
        $total_supplier_payment = SupplierPayment::where('supplier_id', $id)->sum('payment');
        $receivingReport_dues = Receiving::where([['supplier_id', $id],['dues', '>', 0.00], ['status', '=', 0]])->paginate(10);
        $receivingReport_completed = Receiving::where([['supplier_id', $id],['dues', '=', 0.00]])->orWhere([['supplier_id', $id],['status', '=', 1]])->paginate(10);
        $supplier_payments = SupplierPayment::where('supplier_id', $id)->latest()->paginate(3);
        $receiving_payments = DB::select("select * from receivings INNER JOIN receiving_payments ON receivings.id = receiving_payments.receiving_id where supplier_id =".$id." ORDER BY receiving_payments.id DESC LIMIT 5");
